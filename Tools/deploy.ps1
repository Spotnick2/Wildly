<#
    deploy.ps1 - Deploy Wildly into the WoW: Forever AddOns folder.
    ...
    Wildly embeds LibGroupBuffs-1.0. A release gets it from .pkgmeta
    externals; a deploy copies it from the checkout next to this repository
    (../LibGroupBuffs, or -Library) into Wildly\Libs\LibGroupBuffs-1.0,
...
function Deploy-Wildly {
    $dest = Join-Path $AddOnsPath "Wildly"
    ...
    Write-Host "Deploying Wildly -> $dest" -ForegroundColor Cyan
    if (-not (Test-Path $dest)) { New-Item -ItemType Directory -Path $dest | Out-Null }

    $files = @("Wildly.toc") + (Get-ChildItem -LiteralPath $RepoRoot -Filter "*.lua" |
